...
controlToOdom::controlToOdom(ros::NodeHandle nh, ros::NodeHandle private_nh) :
  ...
  use_servo_cmd_(true), publish_tf_(false), x_(0.0), y_(0.0), yaw_(0.0)
{
  ...
  // subscribe to control state and. optionally, servo command
  control_state_sub_ = nh.subscribe("/rugged_car/basics/motor_velocity_feedback", 10, &controlToOdom::controlStateCallback, this);
  if (use_servo_cmd_) {
    servo_sub_ = nh.subscribe("/rugged_car/basics/servo_angle_feedback", 10,
                              &controlToOdom::servoCmdCallback, this);
  }
}

void controlToOdom::controlStateCallback(const rugged_car_basics::MotorStateStamped::ConstPtr& state)
{
...
void controlToOdom::servoCmdCallback(const std_msgs::Float64::ConstPtr& servo)
{
...
inline bool getRequiredParam(const ros::NodeHandle& nh, std::string name, T& value)
{
  ...
  ROS_FATAL("controlToOdom: Parameter %s is required.", name.c_str());
  return false;
}
