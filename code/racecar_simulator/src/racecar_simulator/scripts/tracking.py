...
class Tracking:
    def __init__(self):
        ...
        self.path_sub = rospy.Subscriber(
            '/global_path', Path, self.pathCallback)
        # self.path_sub = rospy.Subscriber(
        #     '/move_base_simple/goal', PoseStamped, self.pointCallback)
        # self.vel_pub = rospy.Publisher(
        #     '/drive', Twist, queue_size=1)
        self.midpose_pub = rospy.Publisher(
            '/mid_goal', PoseStamped, queue_size=1)
    ...
    def updateGlobalPose(self):
        ...
        self.goal_dis = math.hypot(
            self.x-self.path.poses[-1].pose.position.x, self.y-self.path.poses[-1].pose.position.y)
    ...
    def initTracking(self):
        ...
        self.plan_target_ind, _ = self.plan_target_course.search_target_index(
    ...
    def planOnce(self):
        ...
        self.plan_ai = proportional_control(
            self.plan_target_speed, self.plan_state.v)
        self.plan_di, self.plan_target_ind = pure_pursuit_steer_control(
            self.plan_state, self.plan_target_course, self.plan_target_ind)
    ...
    def publishVel(self, zero=False):
        ...
        msg.drive.steering_angle=self.plan_di
        msg.drive.steering_angle_velocity=0
        msg.drive.speed=self.plan_state.v
